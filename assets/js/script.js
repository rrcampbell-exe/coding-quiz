...
function runQuestions() {
    ...
    function iterate() {
        questionHeader.textContent = arr[currentQ].question
        choicesEl.forEach((el, i) => {
            el.textContent = arr[currentQ].choices[i]
        })
    }
    choicesEl.forEach(el => {
        el.addEventListener("click", function (event) {
            let choice = this.textContent
            if (choice === arr[currentQ].answer) {
                console.log("Correct!")
            } else {
...
//     // questionOneHeader.textContent = "True or false: JavaScript and Java are the same thing." instead of text here, could use an array [{}]
